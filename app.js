...
function envoyerDemandeRenouvellement(liste, anneeMax) {
  if (!liste.length) { alert("Aucune personne avec une adresse mail dans cette sélection."); return; }
  const emails = liste.map((p) => p.email).filter(Boolean).join(",");
  const sujet = "Renouvellement de votre photo de casting";
  const corps = `Bonjour,\n\nDans le cadre de la mise à jour de notre base de casting, nous constatons que votre photo actuelle date de ${anneeMax} ou d'avant.\n\nSi vous disposez d'une photo plus récente, merci de nous la faire parvenir par retour de mail (répondez directement à ce message avec la photo en pièce jointe).\n\nMerci d'avance et à bientôt,`;
  window.location.href = `mailto:?bcc=${encodeURIComponent(emails)}&subject=${encodeURIComponent(sujet)}&body=${encodeURIComponent(corps)}`;
}
...
function genererImpressionTrombinoscope(mode) {
  ...
  win.document.write(`
    ...
    <style>
      ...
      .grille{ display:grid; grid-template-columns:repeat(${estReduit ? 5 : 4}, 1fr); gap:${estReduit ? "6px" : "6px"}; }
      .carte{ border:1px solid #999; border-radius:4px; padding:${estReduit ? "4px" : "3px"}; text-align:center; break-inside:avoid; }
      .carte img{ width:100%; height:${estReduit ? "auto" : "95px"}; aspect-ratio:${estReduit ? "3/4" : "auto"}; object-fit:contain; background:#f2f2f2; border-radius:3px; }
      .pas-photo{ width:100%; height:${estReduit ? "auto" : "95px"}; aspect-ratio:${estReduit ? "3/4" : "auto"}; background:#f2f2f2; border-radius:3px; }
      .nom{ font-weight:bold; font-size:${estReduit ? "10px" : "9.5px"}; margin-top:2px; }
      .ligne{ font-size:${estReduit ? "9px" : "7.5px"}; color:#333; line-height:1.15; }
      .ligne.parcours{ font-style:italic; color:#555; text-align:left; margin-top:2px; font-size:7px; }
    </style>
    </head><body>
